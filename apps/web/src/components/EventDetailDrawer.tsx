                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {event.categories.map((cat) => (
                        <span
                          key={cat}
                          className="text-[11px] px-2 py-0.5 rounded bg-red-500/15 text-red-300 border border-red-500/20 capitalize"
                        >
                          {cat}
                        </span>
                      ))}
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {event.actors.map((actor) => (
                        <span
                          key={actor}
                          className="text-[11px] px-2 py-0.5 rounded bg-cyan-500/15 text-cyan-300 border border-cyan-500/20"
                        >
                          {actor}
                        </span>
                      ))}
                ...
                <div className="bg-secondary rounded-lg border border-gray-700 p-3 text-sm space-y-1">
                  {event.location.country && (
                    <div className="flex justify-between">
                      ...
                      <span className="text-gray-200">{event.location.country}</span>
                    </div>
                  )}
                  {event.location.region && (
                    <div className="flex justify-between">
                      <span className="text-gray-500">Region</span>
                      <span className="text-gray-200">{event.location.region}</span>
                    </div>
                  )}
                  {event.location.city && (
                    <div className="flex justify-between">
                      <span className="text-gray-500">City</span>
                      <span className="text-gray-200">{event.location.city}</span>
                    </div>
                  )}
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-gray-200 font-mono text-xs">
                      {(event.location.confidence * 100).toFixed(0)}%
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1">
                  {event.tags.map((tag) => (
                    <span
                      key={tag}
                      className="text-[11px] px-2 py-0.5 rounded bg-gray-700 text-gray-300 border border-gray-600"
                    >
                      {tag}
                    </span>
                  ))}
            ...
            <div className="border-t border-gray-700 pt-4">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div>
                  ...
                  <p className="text-gray-200 capitalize">{event.status}</p>
                </div>
                <div>
                  <span className="text-gray-500">Published</span>
                  <p className="text-gray-200">{format(new Date(event.published_at), 'MMM d, yyyy HH:mm')}</p>
                </div>
                <div>
                  <span className="text-gray-500">Created</span>
                  <p className="text-gray-200">{format(new Date(event.created_at), 'MMM d, yyyy HH:mm')}</p>
                </div>
                <div>
                  <span className="text-gray-500">Event ID</span>
                  <p className="text-gray-200 font-mono text-[10px] truncate">{event.id}</p>
                </div>
              </div>
            </div>
